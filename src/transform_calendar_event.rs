...
/// # Summary
/// Transforms the briefing event. Additionally to the minimum actions changes summary to "Briefing", changes IATA location to ICAO location, and adds alarms at -500 s and -4 ks.
///
/// # Arguments
...
pub async fn transform_briefing(mut calendar_event: icalendar::Event, db: &sqlx::sqlite::SqlitePool) -> icalendar::Event
{
    ...
    calendar_event.alarm(icalendar::Alarm::display(calendar_event.get_summary().unwrap_or_default(), chrono::Duration::seconds(-4000))); // add alarm at -4 ks
    calendar_event.alarm(icalendar::Alarm::display(calendar_event.get_summary().unwrap_or_default(), chrono::Duration::seconds(-500))); // add alarm at -500 s
...
/// # Summary
/// Transforms the deadhead event. Additionally to the minimum actions changes summary format, changes IATA locations to departure ICAO location only, and adds an alarm at -2 ks.
///
/// # Arguments
/// - `calendar_event`: the calendar event to transform
/// - `db`: airport database
...
{
    calendar_event = transform_unknown(calendar_event); // always do minimum before specific actions
    calendar_event.summary(format!("DH {flight_iata}: {} -> {}", try_iata_to_icao(departure_iata.to_owned(), db).await, try_iata_to_icao(destination_iata.to_owned(), db).await).as_str()); // change summary format
    ...
    calendar_event.alarm(icalendar::Alarm::display(calendar_event.get_summary().unwrap_or_default(), chrono::Duration::seconds(-4000))); // add alarm at -4 ks
    calendar_event.alarm(icalendar::Alarm::display(calendar_event.get_summary().unwrap_or_default(), chrono::Duration::seconds(-2000))); // add alarm at -2 ks
...
/// # Summary
/// Transforms the flight event. Additionally to the minimum actions changes summary format, changes IATA locations to departure ICAO location only, and adds an alarm at -1 ks.
///
/// # Arguments
/// - `calendar_event`: the calendar event to transform
/// - `db`: airport database
...
pub async fn transform_flight(mut calendar_event: icalendar::Event, flight_iata: String, departure_iata: String, destination_iata: String, db: &sqlx::sqlite::SqlitePool) -> icalendar::Event
{
    ...
    calendar_event.alarm(icalendar::Alarm::display(calendar_event.get_summary().unwrap_or_default(), chrono::Duration::seconds(-1000))); // add alarm at -1 ks
...
/// # Summary
/// Transforms the ground event. Additionally to the minimum actions changes summary format, changes IATA locations to ICAO location only, and adds alarms at -500 s and -4 ks.
///
/// # Arguments
/// - `calendar_event`: the calendar event to transform
/// - `db`: airport database
...
pub async fn transform_layover(mut calendar_event: icalendar::Event, db: &sqlx::sqlite::SqlitePool) -> icalendar::Event
{
    ...
    calendar_event.alarm(icalendar::Alarm::display(calendar_event.get_summary().unwrap_or_default(), (chrono::Duration::seconds(-4000), icalendar::Related::End))); // add alarm at -4 ks before layover end, at layover end briefing begins
    calendar_event.alarm(icalendar::Alarm::display(calendar_event.get_summary().unwrap_or_default(), (chrono::Duration::seconds(-500), icalendar::Related::End))); // add alarm at -500 s before layover end, at layover end briefing begins
